...
public class ScrollDA : MonoBehaviour {
    ...
    public void search() {
        ...
        if (PlayerPrefs.GetInt("KuisKataDA") > 0 && PlayerPrefs.GetInt("KuisGambarDA") > 0 && PlayerPrefs.GetInt("KuisDengarDA") > 0)
        {
            finishAllQuiz = true;
        }
        ...
        foreach (var item in tempItem)
        {
            ...
            buttonairdarat buttonScript = newButton.GetComponent<buttonairdarat>();
    ...
    void PindahMendengar(int index)
    {
        ...
        IndexButton = index;
        SceneManager.LoadScene("DengarDA");
